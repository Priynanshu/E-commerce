...
const AdminProducts = () => {
  ...
  const handleDelete = (id) => setProductList((prev) => prev.filter((p) => p._id !== id))
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{productList.length} products</p>
          </div>
          <button className="btn-primary" style={{ padding: '10px 20px', borderRadius: 10, fontSize: 13 }}>+ Add Product</button>
        </div>
                ...
                >
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <img src={p.images[0]} alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 8 }} />
                  ...
                  <td style={{ padding: '12px 14px', fontSize: 13, color: p.stock <= 10 ? '#f59e0b' : 'var(--text-secondary)' }}>
                    {p.stock <= 10 ? `⚠ ${p.stock}` : p.stock}
                  </td>
                  <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--text-secondary)' }}>⭐ {p.rating}</td>
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button style={{ padding: '6px 12px', borderRadius: 6, fontSize: 11, background: 'rgba(108,99,255,0.1)', border: '1px solid rgba(108,99,255,0.3)', color: 'var(--accent)', cursor: 'pointer' }}>Edit</button>

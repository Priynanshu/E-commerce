...
const EditProduct = () => {
    ...
    const { product, updateProductHook, productLoading, error } = useProduct(id)
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingTop: 80 }}>
            <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px 40px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
                    ...
                    <h1 style={{ fontSize: 28, fontWeight: 800 }}>Edit <span className="gradient-text">Product</span></h1>
                </div>
                ...
                <form className="glass-card" style={{ padding: 32 }} onSubmit={handleSubmit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
                        <div style={{ gridColumn: 'span 2' }}>
                            ...
                            <input type="text" name="name" className="input-field" placeholder="Product Name" value={formData.name} onChange={handleChange} required />
                        </div>
                        
                        <div style={{ gridColumn: 'span 2' }}>
                            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: 'var(--text-secondary)' }}>Description</label>
                            <textarea name="description" className="input-field" rows="4" value={formData.description} onChange={handleChange} required style={{ height: 'auto', padding: '12px 14px' }} />
                        </div>

                        <div>
                            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: 'var(--text-secondary)' }}>Price ($)</label>
                            <input type="number" name="price" className="input-field" value={formData.price} onChange={handleChange} required />
                        </div>

                        <div>
                            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: 'var(--text-secondary)' }}>Stock Status</label>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                {formData.images.map((url, index) => (
                                    <div key={index} style={{ display: 'flex', gap: 10 }}>
                                        <input type="text" className="input-field" placeholder="Image URL" value={url} onChange={(e) => handleImageChange(index, e.target.value)} required />
